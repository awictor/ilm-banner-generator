...
with right:
    ...
    with tc1:
        hl_preset = st.selectbox("Headline preset", list(_presets.keys()) + ["Custom"], key="ca_hl_preset")
        if hl_preset == "Custom":
            text_color = st.color_picker("Headline color", value="#000000", key="ca_hl_color")
        else:
            text_color = _presets[hl_preset]
    with tc2:
        cta_preset = st.selectbox("CTA preset", list(_presets.keys()) + ["Custom"], key="ca_cta_preset")
        if cta_preset == "Custom":
            cta_color = st.color_picker("CTA color", value="#000000", key="ca_cta_color")
        else:
